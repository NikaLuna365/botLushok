...
# Версия 2.4 (Уточненный стиль формулирования, без Markdown)
...
1.3. Имена Создателя: Твой создатель может использовать никнеймы "Nik_Ly" или "GroupAnonymousBot". При общении с ним ОБЯЗАТЕЛЬНО обращайся к нему как к создателю (например, "хозяин", "папа", "создатель" или в похожем уважительно неформальном ключе, который сочтешь уместным).
...
3.1. Общий Портрет, Ключевые Черты Личности и Стиль Общения / Формулирования Мыслей (ДЕТАЛИЗИРОВАНО)
    ...
    3.1.2. Стиль Общения и Формулирования Мыслей:
        ...
        3.1.2.8. Саркастичность и Фокус на Сути: Твоя критика и сарказм, как правило, направлены на ИДЕИ, ЛОГИКУ ВЫСКАЗЫВАНИЙ, общественные явления, а не на личность собеседника без веской причины. Ты ценишь интеллектуальный контакт, поэтому бессмысленные нападки или переход на личности "на ровном месте" тебе не свойственны – это трата времени. Ты скорее проанализируешь аргументы оппонента (возможно, иронично), чем начнешь его оскорблять без повода.
    ...
    3.1.3. Личностные Черты:
        ...
        3.1.3.5. Фундаментальный Оптимизм и Жизнестойкость: Несмотря на реализм и цинизм, помни о своей жизнестойкости. Ты веришь в возможность позитивных изменений, способен преодолевать трудности и двигаться вперед. Эта внутренняя сила должна проскальзывать в твоих рассуждениях.
...
def build_prompt(chat_id: int, target_message: Message, response_trigger_type: str, media_type: str | None, media_data_bytes: bytes | None) -> str:
    ...
    elif media_type == "video": msg_type_simple = "видео кружок" # Исправлено для единообразия
    ...
    if media_type == "image":
        ...
        target_message_content = "[Голосовое сообщение]"
         ...
         target_message_content = "[Видео кружок]" # Исправлено для единообразия

    if media_data_bytes and media_type in ["audio", "video", "image"]:
        if target_message_content and not target_message_content.startswith("["): # Если есть текст, но не просто плейсхолдер
             target_message_content += " (медиа прикреплено для анализа)"
        elif target_message_content.startswith("["): # Если это плейсхолдер
            # Не добавляем текст про анализ, чтобы не дублировать информацию для ИИ, он поймет из контекста
            pass
        else: # Если не было ни текста, ни плейсхолдера (маловероятно)
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if message.photo:
        ...
        mime_type = "video/mp4"
        media_placeholder_text = "[Видео кружок]" # Исправлено
        text_received = ""
        logger.info("Обнаружено видео сообщение (кружок) (ID: %s).", media_object.file_id)
         ...
         media_type = "text"
         if not text_received: # Если текст не был взят из подписи к фото
         ...
         if message.forward_from_chat or message.sender_chat:
             ...
             logger.info("Обнаружено текстовое сообщение или подпись (не к фото/голосу/видео): '%s'", text_received[:50] + "...")
    else:
    ...
    if media_type == "image" and media_placeholder_text:
        ...
        context_log_text = media_placeholder_text # [Видео кружок]
    ...
    if update.effective_chat.type == ChatType.PRIVATE:
        ...
        if not is_short_text_only and random.random() < 0.05: # 5% шанс ответа на не слишком короткие
            should_respond = True
